...
class MovementManager:
    ...
    def set_control_paused(self, paused: bool) -> None:
        """Pause or resume the 100 Hz set_target loop.

        This is used around SDK-level wake/sleep calls so the movement loop does
        not immediately overwrite those poses.
        """
    ...
    def _handle_command(self, command: str, payload: Any, current_time: float) -> None:
        ...
        if command == "queue_move":
            ...
            if desired_state:
                ...
                self._antenna_unfreeze_blend = 0.0
            self.state.update_activity()
        elif command == "set_control_paused":
            self._control_paused = bool(payload)

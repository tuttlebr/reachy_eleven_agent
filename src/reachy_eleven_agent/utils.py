...
from reachy_eleven_agent.camera_worker import CameraWorker
...
def parse_args() -> Tuple[argparse.Namespace, list]:  # type: ignore
    ...
    parser.add_argument(
        ...
        default=os.getenv("REACHY_AGENT_PROVIDER", "elevenlabs"),
        help="Conversation provider to run. Defaults to ElevenLabs for this app.",
    )
    parser.add_argument(
        "--eleven-agent-id",
        default=os.getenv("ELEVENLABS_AGENT_ID"),
        help="ElevenLabs public agent ID. Defaults to ELEVENLABS_AGENT_ID.",
    )
    parser.add_argument(
        "--eleven-api-key",
        default=os.getenv("ELEVENLABS_API_KEY"),
        help="Optional ElevenLabs API key for private agents. Public agents do not need it.",
    )
    parser.add_argument(
        "--eleven-user-id",
        default=os.getenv("REACHY_ELEVEN_USER_ID"),
        help="Optional end-user ID attached to ElevenLabs conversation history.",
    )
    parser.add_argument(
        "--no-speech-motion",
        default=False,
        action="store_true",
        help="Disable audio-reactive head motion while ElevenLabs speaks.",
    )
    parser.add_argument(
        "--head-tracker",
    ...
    if not args.no_camera:
        ...
        if args.head_tracker is not None:
            if args.head_tracker == "yolo":
                from reachy_eleven_agent.vision.yolo_head_tracker import HeadTracker
        ...
        if args.local_vision:
            try:
                from reachy_eleven_agent.vision.processors import initialize_vision_manager

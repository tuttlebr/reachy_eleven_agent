...
from reachy_eleven_agent.elevenlabs_dashboard import (
    ...
    _remove_autogenerated_root_route,
)
...
def test_dashboard_removes_static_root_route_before_gradio_mount():
    class Route:
    ...
    _remove_autogenerated_root_route(app)

    assert [route.path for route in app.router.routes] == ["/static", "/status"]


def test_dashboard_handlers_bypass_gradio_queue(tmp_path, monkeypatch):
    import gradio as gr
    ...
    def fake_mount(app, blocks, path):
    ...
    mount_elevenlabs_dashboard(App(), make_runtime(tmp_path))

    dependencies = captured["blocks"].config["dependencies"]
    assert captured["path"] == "/"
    assert dependencies
    assert all(dependency["queue"] is False for dependency in dependencies)
    assert dependencies[0]["show_progress"] == "hidden"
